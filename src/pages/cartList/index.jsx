        ...
        <div className="md:col-span-2 space-y-4">
          {cartItems?.length ? (
            cartItems?.map((cartItem) => (
              <CartTile key={cartItem} singleCartItem={cartItem} />
            ))
          ) : (
        ...
        <div className="bg-gray-100 rounded-sm p-4 h-max">
          ...
          <div className="mt-5 flex gap-2">
            <button
              ...
              className="disabled:opacity-60 text-sm px-4 py-3 text-white bg-black font-extrabold"
            >
              Checkout
            </button>
            <button
              onClick={() => navigate("/products")}
              className="text-sm px-4 py-3 text-white bg-black font-extrabold"
            >
              Continue Shopping
            </button>
          </div>
